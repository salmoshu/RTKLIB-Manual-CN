...
demo5作者通常对探测器和基地之间的相对距离感兴趣，所以将其设置为“enu”。如果您对绝对位置感兴趣，请将其设置为“llh”，但请确保在“ant2”设置中设置了确切的基本位置。如果需要精确的z轴测量，请小心使用此设置。只有llh格式会给你一个恒定的z高度，如果rover在恒定的高度。“Enu”和“xyz”是笛卡尔坐标，所以z轴遵循一个平面，而不是地球的曲率。如果基站离探测器较远，这可能会导致特别大的误差，因为曲率会随着距离的增加而增加。
...
伪距测量值与载波相位测量值的标准差之比。demo5作者发现更大的值更适合低成本的接收器，但默认值100通常更适合更昂贵的接收器，因为它们的伪距测量噪声更小。较大的值往往会使卡尔曼滤波器收敛得更快，导致更快的首次Fix，但它也增加了错误Fix的机会。如果增加这个值，则应该将pos2-arthres1设置得足够低，以防止在卡尔曼滤波器有时间收敛之前找到Fix。demo5作者相信增加这个值与增加伪间隔平滑算法上的时间常数具有类似的效果，因为它滤除了伪间隔测量中更多的较高频率，同时保持了低频成分。
...
这是基站天线的位置。如果你只对基地和rover之间的相对距离感兴趣，这个值不需要特别精确。对于后处理，demo5作者通常使用RINEX文件头中的大致基站位置。虽然标记为近似，如果您正在使用来自CORS参考站的rinex文件，它通常是精确的。否则，如果demo5作者想要绝对位置，demo5作者首先根据附近的参考站处理基站数据以获得确切位置，然后使用“llh”或“xyz”选项指定该位置。对于不知道确切基站位置的实时处理，demo5作者使用“单一”选项，它使用数据中的单一解决方案来获得基站位置的粗略估计。

### ant2-maxaveep = 1
当“postype”设置为“single”时，指定用于确定基站位置的平均样本数。

demo5作者将其设置为1，以防止在卡尔曼滤波器开始收敛后基站位置发生变化，因为这似乎会导致较长时间的首次Fix。在大多数情况下，对于后处理，基站位置将来自RINEX文件头，因此您不会使用此设置。但是，如果您正在使用RTCM文件，您甚至可能需要进行后处理。

## MISC
### misc-timeinterp =off,on
插入基站观测值。如果基站观测样本时间大于5秒，demo5作者通常将其设置为“开”。
